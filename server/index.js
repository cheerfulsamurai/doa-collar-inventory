...
async function initDB() {
  ...
  if (parseInt(rows[0].count) === 0) {
    for (const c of SEED) {
      await pool.query('INSERT INTO collars (id,serial,type,status,client,date,trainer) VALUES ($1,$2,$3,$4,$5,$6,$7)', [c.id, c.serial, c.type, c.status, c.client, c.date, c.trainer]);
    }
...
app.get('/api/collars', async (req, res) => {
  try { const { rows } = await pool.query('SELECT * FROM collars ORDER BY created_at ASC'); res.json(rows); }
  catch (e) { console.error(e); res.status(500).json({ error: e.message }); }
});

app.post('/api/collars', async (req, res) => {
  const { id, serial, type } = req.body;
  try { const { rows } = await pool.query('INSERT INTO collars (id,serial,type,status,client,date,trainer) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *', [id, serial, type, 'Received', '', '', '']); res.json(rows[0]); }
  catch (e) { console.error(e); res.status(500).json({ error: e.message }); }
});
...
app.patch('/api/collars/:id', async (req, res) => {
  ...
  try { const { rows } = await pool.query('UPDATE collars SET status=$1, client=$2, date=$3, trainer=$4 WHERE id=$5 RETURNING *', [status, client || '', date || '', trainer || '', id]); res.json(rows[0]); }
  catch (e) { console.error(e); res.status(500).json({ error: e.message }); }
});
